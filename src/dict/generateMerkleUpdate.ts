...
import { DictionaryKeyTypes, Dictionary } from './Dictionary';
...
export function generateMerkleUpdate<K extends DictionaryKeyTypes, V>(
    ...
    const oldProof = generateMerkleProof(dict, key).refs[0];
    dict.set(key, newValue);
    const newProof = generateMerkleProof(dict, key).refs[0];
